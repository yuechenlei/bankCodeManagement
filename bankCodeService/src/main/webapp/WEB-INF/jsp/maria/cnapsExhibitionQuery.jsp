...
<body style="overflow:auto;">
	...
	<br>
	<form:form id="form" action="${pageContext.request.contextPath}/maria/cnapsExhibitionQuery.htm" method="post"
	target="queryResult" commandName="cnaps">
		<table cellpadding="0" cellspacing="1" align="center"
			...
			<tr>
				<th>创建日期</th>
				<td>
					<input type="text" name="createDateStart" id="createDateStart" class="Wdate width80" autocomplete='off'/>
					-
					<input type="text" name="createDateEnd" id="createDateEnd" class="Wdate width80" autocomplete='off'/>
				</td>
				<th>最后更新日期</th>
				<td>
					<input type="text" name="lastModifyDateStart" id="lastModifyDateStart" class="Wdate width80" autocomplete='off'/>
					-
					<input type="text" name="lastModifyDateEnd" id="lastModifyDateEnd" class="Wdate width80" autocomplete='off'/>
				</td>
				<th>修改次数</th>
				<td><input type="text" id="vision"name="vision" value="0" ></td>
			</tr>

		</table>
		<br/>
		<div>
		<input type="button" class="global_btn" style="margin-left:45%" value="查  询" onclick="javascript:cnapsExhibitionQuery()" />
		<input type="reset" class="global_btn" style="margin:0px 20px auto" value="重  置" />
		<input type="hidden" id="cnapsTemp" value="" name="cnaps" />
		</div>
	</form:form>
	...
	<div>
	<form action="${pageContext.request.contextPath}/maria/downloadTemplate.htm" style="display:none;" target="exportResult" method="post" id="downloadForm">
...
<script type="text/javascript">
	$(function() {
		...
		$("#lastModifyDateStart").datepicker({
			dateFormat : "yy-mm-dd 00:00:00",
			changeMonth : true,
			changeYear : true,
			showButtonPanel: true,
	...
	function cnapsExhibitionQuery(){
		//var fd = new FormData($("#form")[0]);
		//$("#cnapsTemp").attr("value", fd);
		$("#form").submit();
	}
</script>
</html>