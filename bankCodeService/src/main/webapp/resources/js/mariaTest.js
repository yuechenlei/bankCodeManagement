...
function addCnaps(){
	...
	$.ajax({
		type:"POST",
		url: contextPath+"/maria/cnapsIncrease.json",
		data: fd,
		dataType:"json",
		cache: false,  // 不用缓存
...
function exportResult(){
	$("#form").attr("action", contextPath+"/maria/cnapsExport.htm");
	$("#form").attr("target", "exportResult");
	$("#form").submit();

	$("#form").attr("action", contextPath+"/maria/cnapsExhibitionQuery.htm");
...
function cnapsTotalExport(){
	...
	$("#dialog-message-1").dialog({
		...
		buttons: {
			"确定": function() {
			$( this ).dialog( "close" );
			$("#form").attr("action", contextPath+"/maria/cnapsTotalExport.htm");
			$("#form").attr("target", "exportResult");
			$("#form").submit();
...
function cnapsModify(cncode){
	...
	$("#"+cncode+"2").html('<input type="text" size="55" id='+cncode+'b'+' value='+$("#"+cncode+"2").html()+' />');
	$("#"+cncode+"3").html('<input type="text" size="15" id='+cncode+'c'+' value='+$("#"+cncode+"3").html()+' />');
	$("#"+cncode+"4").html('<input type="text" size="5" id='+cncode+'d'+' value='+$("#"+cncode+"4").html()+' />');
	$("#"+cncode+"5").html('<input type="text" size="5" id='+cncode+'e'+' value='+$("#"+cncode+"5").html()+' />');
	$("#"+cncode+"6").html('<input type="text" size="5" id='+cncode+'f'+' value='+$("#"+cncode+"6").html()+' />');
	$("#"+cncode+"7").html('<input type="hidden" />');
	$("#"+cncode+"8").html('<input type="hidden" />');
	$("#"+cncode+"9").html('<input type="hidden" />');
}

// 确认修改
function cnapsConfirm(cncode,cnname,cnclearingBankCode,cnclearingBankLevel,
		cnproviderCode,cnadCode,cncreateDate,cnlastModifyDate,cnvision){
	...
	if(code != cncode){
		...
		$.ajax({
			type:"POST",
			async: false,
			url: contextPath+"/maria/cnapsDel.json",
		...
		$.ajax({
			type:"POST",
			url: contextPath+"/maria/confirmModify.json",
			data:{"code":code,"name":name,"clearingBankCode":clearingBankCode,"clearingBankLevel":clearingBankLevel,"providerCode":providerCode,"adCode":adCode,"vision":cnvision},
			dataType:"json",
			success:function(msg){
				if(msg.result == "success"){
					$("#"+cncode+"1").html(code);
					...
					$("#"+cncode+"7").html(cncreateDate);
					$("#"+cncode+"8").html(cnlastModifyDate);
					$("#"+cncode+"9").html(cnvision);
					...
					$("#"+code+"Con").attr("href","javascript:cnapsConfirm("+code+","+"'"+name+"'"+","+clearingBankCode+","+clearingBankLevel+","+"'"+providerCode+"'"+
							","+adCode+","+"'"+cncreateDate+"'"+","+"'"+cnlastModifyDate+"'"+","+cnvision+")");
					$("#"+code+"Can").attr("href","javascript:cnapsCancel("+code+","+"'"+name+"'"+","+clearingBankCode+","+clearingBankLevel+","+"'"+providerCode+"'"+
							","+adCode+","+"'"+cncreateDate+"'"+","+"'"+cnlastModifyDate+"'"+","+cnvision+")");
					...
					$("#"+cncode+"7").attr("id",code+"7");
					$("#"+cncode+"8").attr("id",code+"8");
					$("#"+cncode+"9").attr("id",code+"9");
		...
		$.ajax({
			type:"POST",
			url: contextPath+"/maria/modify.json",
			data:{"code":cncode,"name":name,"clearingBankCode":clearingBankCode,"clearingBankLevel":clearingBankLevel,"providerCode":providerCode,"adCode":adCode,"vision":cnvision},
			dataType:"json",
			success:function(msg){
				if(msg.result == "success"){
					$("#"+cncode+"1").html(code);
					...
					$("#"+cncode+"Con").attr("href","javascript:cnapsConfirm("+cncode+","+"'"+name+"'"+","+clearingBankCode+","+clearingBankLevel+","+"'"+providerCode+"'"+
							","+adCode+","+"'"+cncreateDate+"'"+","+"'"+cnlastModifyDate+"'"+","+cnvision+")");
					$("#"+cncode+"Can").attr("href","javascript:cnapsCancel("+cncode+","+"'"+name+"'"+","+clearingBankCode+","+clearingBankLevel+","+"'"+providerCode+"'"+
							","+adCode+","+"'"+cncreateDate+"'"+","+"'"+cnlastModifyDate+"'"+","+cnvision+")");
...
// 取消修改
function cnapsCancel(cncode,name,clearingBankCode,clearingBankLevel,providerCode,adCode,createDate,lastModifyDate,vision){
	$("#"+cncode+"Mod").css("display","inline");
	$("#"+cncode+"Del").css("display","inline");
	$("#"+cncode+"Line2").css("display","inline");
	$("#"+cncode+"Line").css("display","none");
	...
	$("#"+cncode+"5").html(providerCode);
	$("#"+cncode+"6").html(adCode);
	$("#"+cncode+"7").html(createDate);
	$("#"+cncode+"8").html(lastModifyDate);
	$("#"+cncode+"9").html(vision);
...
function batchModify(){
	...
	$.ajax({
		type:"POST",
		url: contextPath+"/maria/batchModification.json",
		data: fd,
		dataType:"json",
		cache: false,  // 不用缓存
		processData: false,  // 不处理发送的数据
