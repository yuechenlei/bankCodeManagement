...
import com.xiaoqingxin.bankCodeService.redisService.RedisRedPacketService;

@Service
public class RedisRedPacketServiceImpl implements RedisRedPacketService {
	private static final Logger logger = LoggerFactory.getLogger(RedisRedPacketServiceImpl.class);
	...
	public void saveUserRedPacketByRedis(Long redPacketId, Double unitAmount) {
		...
		logger.info("异步保存数据结束，耗时" + (end - start) + "毫秒，共" + count + "条记录被保存。");
	}
	...
	private int executeBatch(List<UserRedPacket> userRedPacketList) {
		...
		try {
			...
			/********* 错误处理逻辑 ********/
			throw new RuntimeException("使用JDBC批量处理Redis缓存数据，执行程序错误");
		} finally {
			try {
